...
const isCourseFormDirty =
  JSON.stringify(newCourse) !== JSON.stringify(initialCourseData) || !!thumbnail;
...
const fetchCategories = async () => {
  try {
    ...
    setFeedback({ type: "error", message: e.message || "Failed to load categories" });
  } finally {
  ...
  try {
    ...
    setCategories((prev) => {
      const exists = prev.some(
        (category) => category.name.toLowerCase() === created.name.toLowerCase()
      );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setError("");
        const data = await apiGet(`/api/admin/courses${query ? `?search=${encodeURIComponent(query)}` : ""}`);
  ...
  setShowModal(true);
     ...
     >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Courses" value={loadingStats ? "…" : stats?.totalCourses ?? 0} />
        <StatCard label="Total Lessons" value={loadingStats ? "…" : stats?.totalLessons ?? 0} />
        <StatCard label="Total Enrolled" value={loadingStats ? "…" : stats?.totalEnrolled ?? 0} />
        <StatCard label="Avg Rating" value={loadingStats ? "…" : (stats?.avgRating ?? "N/A")} />
      </div>
...
<div className="rounded-xl border border-gray-200 dark:border-gray-700 p-3 bg-gray-50 dark:bg-black/20">
  ...
  <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
    {loadingCategories ? "Loading categories..." : `${categories.length} categories available`}
  </p>
</div>
