...
exports.createLesson = async (req, res) => {
	try {
		...
		await lesson.save();
		await deleteCache(`lessons:${courseId}`); // bust cache
...
exports.getLessonsByCourse = async (req, res) => {
	try {
		const cacheKey = `lessons:${req.params.courseId}`;
		...
		await setCache(cacheKey, lessons, 600); // 10 minutes
		res.json(lessons);
	} catch (error) {
		res.status(500).json({ message: error.message });
	}
...
exports.updateLesson = async (req, res) => {
	try {
		...
		if (req.files?.video) lesson.videoFile = "/uploads/" + req.files.video[0].filename;

		await lesson.save();
		await deleteCache(`lessons:${lesson.courseId}`); // bust cache
