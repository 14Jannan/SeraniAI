...
describe("billingController", () => {
  ...
  describe("forceActivateSubscription (admin override)", () => {
    // NOTE: authentication/authorization (protect + authorize("admin")) is
    // enforced entirely at the route layer (subscriptionRoutes.js), the
    // same pattern used for the existing admin subscription routes - this
    // suite tests the handler's own behavior once a request has already
    // passed that gate.

    it("returns 400 for a malformed subscription id", async () => {
      const req = { params: { id: "not-an-id" }, user: { _id: "admin1" } };
      const res = mockRes();

      await forceActivateSubscription(req, res);
    ...
    it("is a no-op (doesn't re-save or re-sync) when already Active", async () => {
      const subscription = {
        status: "Active",
        save: vi.fn().mockResolvedValue(undefined),
      };
      ...
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith(
        expect.objectContaining({ message: "Subscription is already active" }),
      );
    });

    it("returns 400 when the plan on the subscription can't be resolved", async () => {
      const subscription = {
        status: "Pending",
        plan: "Something Unrecognized",
        planCode: null,
        save: vi.fn().mockResolvedValue(undefined),
      };
      vi.spyOn(Subscription, "findById").mockResolvedValue(subscription);
    ...
    it("activates the subscription and upgrades the user's role, bypassing PayHere entirely", async () => {
      const subscription = {
        _id: "sub1",
        status: "Pending",
        userId: "u1",
      ...
      vi.spyOn(Subscription, "findById")
        ...
        .mockReturnValueOnce({
          lean: vi.fn().mockResolvedValue({ ...subscription, status: "Active" }),
        });
      const user = { role: "user", enterpriseId: null, save: vi.fn().mockResolvedValue(undefined) };
      vi.spyOn(User, "findById")
        .mockResolvedValueOnce(user) // inside syncUserRoleFromPlanCode
        .mockReturnValueOnce({ lean: vi.fn().mockResolvedValue({ ...user, role: "(Pro)PlanUser" }) });
      const payHereSpy = vi.spyOn(payHereService, "getPaymentByOrderId");

      const req = {
        params: { id: "507f1f77bcf86cd799439011" },
        user: { _id: "admin1", email: "admin@seraniai.com" },
      };
      const res = mockRes();

      await forceActivateSubscription(req, res);

      // The entire point of this endpoint is that it does NOT touch PayHere.
      expect(payHereSpy).not.toHaveBeenCalled();
      expect(subscription.status).toBe("Active");
      expect(subscription.payHereStatus).toBe("ACTIVE");
      expect(subscription.save).toHaveBeenCalled();
      expect(user.role).toBe("(Pro)PlanUser");
      expect(res.status).toHaveBeenCalledWith(200);
    });

    it("returns 500 without leaking internals when something unexpected fails", async () => {
      vi.spyOn(Subscription, "findById").mockRejectedValue(new Error("DB down"));
      const req = {
        params: { id: "507f1f77bcf86cd799439011" },
        user: { _id: "admin1" },
      };
      ...
      expect(res.json).toHaveBeenCalledWith(
        expect.objectContaining({ message: expect.not.stringContaining("DB down") }),
      );
    });
  });
});
